...
# This works by finding all matches of `{{...}}` and parsing them.
# If a match is a block, it will find the closing tag and extract the block content.
# If a match is not a block, it will replace it with the value or run the function required.
# It will continue in this manner until no more matches are found.
be() {
  local input result
  input=$(cat -)
  result="$input"
